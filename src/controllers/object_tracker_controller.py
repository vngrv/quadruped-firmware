...
flags.DEFINE_string('weights', './checkpoints/yolov4-416',
                    # Change second argument to './checkpoints/yolov4-416' to use non-tiny model
                    'path to weights file')
flags.DEFINE_integer('size', 416, 'resize images to')
# Change second argument to 'False' to use non-tiny model (approx. 100% speedup using tiny)
...
# Set the default value to your raspberry pi for easier startup.
flags.DEFINE_string('pi_ip', '192.168.2.135',
                    "the ip of the quadruped's raspberry pi")
flags.DEFINE_integer(
    'pi_port', 5000, "the port of the quadruped's raspberry pi")
flags.DEFINE_boolean('return_to_zero', False,
                     'slowly reduce momentum to zero if not controlling')
...
def main(_argv):
    ...
    while not close:
        try:
            ...
            count = len(names)
            if FLAGS.count:
                cv2.putText(image, "Objects being tracked: {}".format(
            ...
            # draw bbox on screen
                ...
                cv2.rectangle(image, (int(bbox[0]), int(bbox[1]-30)), (int(bbox[0])+(
                    len(class_name)+len(str(track.track_id)))*17, int(bbox[1])), color, -1)
                cv2.putText(image, class_name + "-" + str(track.track_id),
                            (int(bbox[0]), int(bbox[1]-10)), 0, 0.75, (255, 255, 255), 2)

            # Grab bbox centers
                centers[i] = (int((bbox[0] + bbox[2])/2),
                              int((bbox[2] + bbox[3])/2))
            ...
            norm_detections = [(bbox, score)
                               for bbox, score in zip(norm_bboxes, scores)]
